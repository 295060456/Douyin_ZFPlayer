//
//  DDNetworkingAPI+CommunityApi.m
//  DouDong-II
//
//  Created by Jobs on 2021/2/17.
//

#import "DDNetworkingAPI+CommunityApi.h"
/*
 * App端接口文档
...
@implementation DDNetworkingAPI (CommunityApi)
/// 创作者排行列表
NSString *creationListGET;
+(void)creationListGET:(id)parameters
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.url = [request.server stringByAppendingString:NSObject.creationListGET.url];
...
/// 收益排行列表
NSString *incomeListGET;
+(void)incomeListGET:(id)parameters
        successBlock:(MKDataBlock)successBlock
        failureBlock:(MKDataBlock)failureBlock{
    NSDictionary *parameterss = @{};
    NSDictionary *headers = @{};
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.server = NSObject.BaseUrl;
        request.url = [request.server stringByAppendingString:NSObject.incomeListGET.url];
        NSLog(@"request.URLString = %@",request.url);
...
/// 人气视频列表
NSString *PopularityVideos;
+(void)popularityVideosGET:(id)parameters
              successBlock:(MKDataBlock)successBlock
              failureBlock:(MKDataBlock)failureBlock{
    NSDictionary *parameterss = @{};
    NSDictionary *headers = @{};
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.server = NSObject.BaseUrl;
        request.url = [request.server stringByAppendingString:NSObject.popularityVideosGET.url];
        NSLog(@"request.URLString = %@",request.url);
