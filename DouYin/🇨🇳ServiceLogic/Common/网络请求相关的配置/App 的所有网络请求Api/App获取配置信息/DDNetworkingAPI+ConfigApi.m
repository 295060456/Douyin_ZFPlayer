...
#import "DDNetworkingAPI+ConfigApi.h"
/*
 * App端接口文档
 ...
 */
@implementation DDNetworkingAPI (ConfigApi)
...
//        request.headers = headers;//与公共配置 Headers 兼容
        request.retryCount = LONG_LONG_MAX;//请求失败 单次请求 重新连接次数 优先级大于 全局设置，不影响其他请求设置
...
//            request.keepType=ZBResponseKeepNone;
        ...
        NSLog(@"error = %@",error);
        if (error) {
            [WHToast toastMsg:[NSObject.refreshGET.funcName stringByAppendingString:error.description]];
        }
