//
//  DDNetworkingAPI+PostCommentApi.m
//  DouDong-II
//
//  Created by Jobs on 2021/1/8.
//

#import "DDNetworkingAPI+PostCommentApi.h"
...
@implementation DDNetworkingAPI (PostCommentApi)

#pragma mark —— App帖子评论相关接口
/// 评论 GET
NSString *commentAddPOST;
+(void)commentAddPOST:(id)parameters
...
//    NSDictionary *headers = @{};
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.url = [request.server stringByAppendingString:NSObject.commentAddPOST.url];
...
///  全部评论列表 GET
NSString *commentAllListGET;
+(void)commentAllListGET:(id)parameters
            successBlock:(MKDataBlock)successBlock{
//    NSDictionary *parameterss = @{};
//    NSDictionary *headers = @{};
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.server = NSObject.BaseUrl;
        request.url = [request.server stringByAppendingString:NSObject.commentAllListGET.url];
        NSLog(@"request.URLString = %@",request.url);
...
/// 评论列表 GET
NSString *commentListGET;
+(void)commentListGET:(id)parameters
         successBlock:(MKDataBlock)successBlock{
//    NSDictionary *parameterss = @{};
//    NSDictionary *headers = @{};
    ...
    [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
        ...
        request.server = NSObject.BaseUrl;
        request.url = [request.server stringByAppendingString:NSObject.commentListGET.url];
        NSLog(@"request.URLString = %@",request.url);
