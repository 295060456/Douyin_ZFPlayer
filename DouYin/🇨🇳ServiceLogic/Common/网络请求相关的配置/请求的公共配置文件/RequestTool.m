...
#pragma mark —— Token
    ...
    [ZBRequestManager setupBaseConfig:^(ZBConfig * _Nullable config) {
        ...
        config.requestSerializer = ZBHTTPResponseSerializer; //全局设置 请求格式 默认响应数据为二进制格式
        config.responseSerializer = ZBJSONResponseSerializer; //全局设置 响应格式 默认JSON
        config.timeoutInterval = 60;//超时时间  优先级 小于 单个请求重新设置
...
//        }
    ...
    [ZBRequestManager setResponseProcessHandler:^id(ZBURLRequest * _Nullable request,
        ...
        NSLog(@"成功回调 数据返回之前");
        if ([request.userInfo[@"info"] isEqualToString:@"DDInvitationDetailsInputViewnetworking_commentAddGET"]) {//分辨接口，预防个别接口的处理方式和大众不太一样
...
//                }
    ...
    [ZBRequestManager setErrorProcessHandler:^(ZBURLRequest * _Nullable request,
        ...
        if (error.code == NSURLErrorCancelled){
            NSLog(@"请求取消❌------------------");
            [WHToast toastMsg:@"请求取消"];
        }else if (error.code == NSURLErrorTimedOut){
            NSLog(@"请求超时");
            [WHToast toastMsg:@"请求超时"];
        }else{
            NSLog(@"请求失败");
            [WHToast toastMsg:[@"请求失败:" stringByAppendingString:error.description]];
        }
    }];
#pragma mark —— 证书设置
    /**
